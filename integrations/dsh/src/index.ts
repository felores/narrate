...
// Type-only side effects: resolve ctx.systemPrompt / ctx.tools / ctx.skills
// and the session/event + session/disposed event declarations.
...
/** Resolved plugin configuration after env + schema defaults. */
...
async function serverVoice(url: string, kind: 'session' | 'narrate'): Promise<ServerVoicePair | null> {
  try {
    ...
    if (!h.default_voice) return null
 ...
 */
async function speak(resolved: ResolvedConfig, text: string, kind: 'session' | 'narrate'): Promise<void> {
  ...
  try {
    ...
    if (resolved.voiceOverride !== '') {
      ...
      if (voice !== null) {
        ...
        if (voice.provider !== undefined) payload.provider = voice.provider
      }
    }
...
function bundledSkillBody(): string {
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  if (resolved.autoVoice) {
    const spoken = new Map<Session, Set<string>>()
    ctx.on('session/event', (session, event) => {
      ...
      if (seen.has(event.data.message.id)) return
      seen.add(event.data.message.id)
      const marker = extractBotMarker(textOf(event.data.message))
      if (marker !== null) void speak(resolved, marker, 'session')
    })
        ...
        defineTool({
          ...
          parameters: {
            text: {
              ...
              required: true,
              description:
                'The text to speak aloud. Keep concise (under 100 words / 500 chars); ' +
    ...
    if (body !== '') {
      ctx.inject(['skills'], (skillsCtx) => {
        skillsCtx.skills.register({
          name: 'narrate',
          description:
            'Use narrate — the local TTS gateway. Setup, providers, voices, ' +
