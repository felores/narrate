...
`narrate` is a provider-agnostic TTS gateway. Single Bun process exposing three interfaces (HTTP, MCP, CLI) over eight TTS providers (ElevenLabs, OpenAI, Gemini, xAI, Soniox, Fish Audio, Voicebox, system). Designed to drop into any AI coding harness.
...
- **Voice catalogs are live, not curated.** ElevenLabs voices are fetched from `/v1/voices` with the account key from `~/.env` (real names — ElevenLabs renamed premade voices, e.g. `EXAVITQu4vr4xnSDxMaL` is now "Sarah", not "Bella"). Soniox fetches `tts-rt-v2` voices from `/v1/tts-models`, falls back only to Adrian, and stays flat so every voice keeps its description. Fish Audio models are fetched **paginated** (the public list is 1000+ and churns) and cached at `~/.cache/narrate/fish-models.json` for 15 min, otherwise a previously-selected voice falls out of the window and the header shows its raw id. Both ElevenLabs and Fish fetches use an unverified SSL context — system python's cert store can't verify `api.fish.audio`/`api.elevenlabs.io` (`CERTIFICATE_VERIFY_FAILED`).
...
- **Update row runs `narrate-menubar-update.sh`** — Service section shows `🔄 Actualizar narrate (vX.Y.Z)`; the version comes from `/health.version` (server reads `package.json`). The helper does `git pull --ff-only` + `bun install` for git checkouts, or re-runs `install.sh` in binary mode for `~/.local/share/narrate` installs, then restarts the launchd service and notifies via osascript (log at `/tmp/narrate-update.log`). `--ff-only` so local work is never clobbered.
...
- **Config helper modes:** `narrate VOICE PROVIDER`, `auto VOICE PROVIDER`, `auto-same` (clear session pair), `narrate-provider VOICE PROVIDER`, and `auto-provider VOICE PROVIDER`. `select VOICE PROVIDER` remains for external legacy callers. SwiftBar 2.1.0/2.1.1 relaunches only for provider-changing modes plus legacy `select`; ordinary voice changes URL-refresh. Newer SwiftBar versions always URL-refresh.
...
- DeepSeek Harness integration → `integrations/dsh/` (Cordis plugin: auto-voice on `assistant/message` + `narrate_speak` tool + system-prompt convention + bundled skill; installer runs `dsh plugin add`).
